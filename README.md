...
This project showcases an AI assistant chat application utilizing three distinct communication methods: standard HTTP, Server-Sent Events (SSE), and WebSocket with streaming capabilities. The frontend connects to two separate Flask servers based on the chosen communication method. Both servers interface with an OpenAI-powered travel assistant, offering automated solutions for travel inquiries such as flight schedule searches and visa requirement checks. Additionally, the application integrates with an external chat system, providing a seamless chat experience with both AI travel assistants and human consultants.
